...
class LuciaMemory:
    def __init__(self):
        self.connection=sqlite3.connect('lucia.db')
        self.cursor=self.connection.cursor()
        self.create_table()
    def create_table(self):
        # in this version lucia has a single database with 3 tables
        # 1 gernal memory
        self.cursor.execute("""
    ...
    def save_memory(self,key,value,category='general'):
        self.cursor.execute("SELECT id FROM memories WHERE key = ? AND category = ?",(key,category))
        ...
        if existing:
            ...
            print(f'[MEMORY] is updated: {key} = {value}')
        else:
            #new created if not exist 
            self.cursor.execute("""
    ...
    def get_memory(self,key,category=None):
        #recall memory
        if category:
            #search through a specific category told by user
            self.cursor.execute("SELECT value FROM memories WHERE key=? AND category=?",
            (key,category))
        else:
            #Randomly search
        ...
        if result:
            #it will tell how much values it checked
            self.cursor.execute("UPDATE memories SET access_count=access_count +1 WHERE key=?",(key,))
            self.connection.commit()
    ...
    def search_memory(self,search_term):
        ...
        if  results:
            print(f"[MEMORY] found {len(results)} results for '{search_term}'")
            return[
                {
                    "key":r[0],
                    "value":r[1],
    ...
    def get_all_memories(self,category=None):
        #it show all memories
        if category:
            self.cursor.execute(
                "SELECT key, value, category, created_at FROM memories WHERE category =?",
                (category,)
            )
        else:
            self.cursor.execute("SELECT key, value, category, created_at FROM memories")
        results=self.cursor.fetchall()
        return[
            {
                "key":r[0],
    ...
    def save_conversation(self, user_message, lucia_response,emotion_detected, intent_detected):
        self.cursor.execute("""
    ...
    def get_recent_conversations(self,limit=5):
        self.cursor.execute("""
            SELECT user_message, lucia_response, emotion_detected,intent_detected,timestamp FROM conversations
...
#     # Test save
#     mem.save_memory("name", "Mahadi", "personal_info")

#     # Test get
#     print(mem.get_memory("name"))

#     # Test search
#     print(mem.search_memory("Mahadi"))

#     # Test all
#     print(mem.get_all_memories())
